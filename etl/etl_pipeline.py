"""
ETL Pipeline — Retail Sales Project (with Region)
...
def transform(df):
    ...
    df["profit_margin_pct"] = 30.0
    log.info("  Transformation complete")
    return df

def load(df, db_path, schema_sql):
    ...
    cur.executemany(
        "INSERT OR REPLACE INTO dim_region(region_id, region_name, state) VALUES(?,?,?)",
        regions)
    region_map = {"Andhra Pradesh": 1, "Karnataka": 2, "Goa": 3}
    df["region_id"] = df["region"].map(region_map)
    log.info(f"  dim_region:    3 rows")

    # dim_date
    date_df = df[["date_key","year","quarter","month","month_name",
                  "week","day_of_week","day_name","is_weekend"]].drop_duplicates("date_key")
    ...
    # fact_sales
    fact_df = df[["transaction_id","date_key","customer_id","product_id","region_id",
                  "quantity","price_per_unit","total_amount","profit"]]
    ...
    # agg_region_performance (NEW)
    reg_perf = df.groupby(["year_month","region"]).agg(
        revenue         = ("total_amount", "sum"),
        orders          = ("transaction_id","count"),
        profit          = ("profit",       "sum"),
        avg_order_value = ("total_amount", "mean"),
    ).reset_index().rename(columns={"region":"region_name"})
    reg_perf.to_sql("agg_region_performance", conn, if_exists="replace", index=False)
    log.info(f"  agg_region_performance: {len(reg_perf):,} rows")
